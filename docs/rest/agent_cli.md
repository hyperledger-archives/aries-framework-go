...
Flags:
  ...
  -q, --database-type string               The type of database to use for everything except key storage. Supported options: mem, leveldb.  Alternatively, this can be set with the following environment variable: ARIESD_DATABASE_TYPE
...
$ go build
$ ./aries-agent-rest start --api-host localhost:8080 --database-type mem --database-prefix aries --inbound-host http@localhost:8081,ws@localhost:8082 --inbound-host-external http@https://example.com:8081,ws@ws://localhost:8082 --webhook-url localhost:8082 --agent-default-label MyAgent
```
