...
const (
	certPrefix        = "../../../../pkg/didcomm/transport/http/testdata/crypto/"
...
func TestRemoteKeyStore(t *testing.T) {
	...
	hf := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		err = processPOSTRequest(w, r, defaultKeyStoreID, defaultKID, marshalledPubKey, kmsapi.ECDSAP256TypeIEEEP1363)
	...
	t.Run("Create API error", func(t *testing.T) {
		...
		_, _, err = tmpKMS.Create(kmsapi.ED25519Type)
		require.Contains(t, err.Error(), "api error msg")
	})
	...
	t.Run("Create Key failure", func(t *testing.T) {
		...
		_, _, err = tmpKMS.CreateAndExportPubKeyBytes(kmsapi.ED25519Type)
		require.Contains(t, err.Error(), "posting Create key failed")
	...
	t.Run("New, Create, Get, Export/Import success, all other functions should fail", func(t *testing.T) {
		...
		kid, keyURL, err := remoteKMS.Create(kmsapi.ECDSAP256TypeIEEEP1363)
		require.NoError(t, err)
		...
		t.Run("CreateKey json marshal failure", func(t *testing.T) {
			...
			_, _, err = remoteKMS2.Create(kmsapi.ED25519Type)
		...
		require.Equal(t, kmsapi.ECDSAP256TypeIEEEP1363, kt)
		...
		t.Run("ExportPubKeyBytes json unmarshal failure", func(t *testing.T) {
			...
			kid1, keyURL1, e := remoteKMS3.Create(kmsapi.ED25519Type)
			require.NoError(t, e)
		...
		nKID, _, err := remoteKMS.CreateAndExportPubKeyBytes(kmsapi.AES128GCMType)
		require.NoError(t, err)
		...
		_, _, err = remoteKMS.Rotate(kmsapi.AES128GCMType, "")
		require.EqualError(t, err, "function Rotate is not implemented in remoteKMS")

		_, err = remoteKMS.PubKeyBytesToHandle(nil, kmsapi.AES128GCMType)
...
func TestCreateKeyWithLocationInResponseBody(t *testing.T) {
	...
	kid1, keyURL1, err := remoteKMS.Create(kmsapi.ED25519Type)
	require.NoError(t, err)
	require.Equal(t, defaultKID, kid1)
	require.Contains(t, keyURL1, fmt.Sprintf("%s/keys/%s", defaultKeystoreURL, defaultKID))

	// no error if options passed
	_, _, err = remoteKMS.Create(kmsapi.ED25519Type, kmsapi.WithAttrs([]string{"attr1"}))
	require.NoError(t, err)
	_, _, err = remoteKMS.CreateAndExportPubKeyBytes(kmsapi.ED25519Type, kmsapi.WithAttrs([]string{"attr2"}))
	require.NoError(t, err)

	remoteKMS.unmarshalFunc = failingUnmarshal
	_, _, err = remoteKMS.Create(kmsapi.ED25519Type)
	require.Contains(t, err.Error(), "unmarshal failed")
	require.Contains(t, err.Error(), "failingUnmarshal always fails")
}
...
func TestRemoteKeyStoreWithHeadersFunc(t *testing.T) {
	...
	t.Run("test New with valid http header func option", func(t *testing.T) {
		...
		kid, keyURL, e := remoteKMS.Create(kmsapi.ED25519Type)
		require.NoError(t, e)
		require.Equal(t, defaultKID, kid)
...
func TestImportPrivateKey(t *testing.T) {
	...
	keyID, keyURL, err := remoteKMS.ImportPrivateKey(privateKey, kmsapi.ED25519Type)
	require.NoError(t, err)
	require.Equal(t, defaultKID, keyID)
	require.Contains(t, keyURL, fmt.Sprintf("%s/keys/%s", defaultKeystoreURL, defaultKID))

	_, _, err = remoteKMS.ImportPrivateKey([]byte("invalid key bytes"), kmsapi.ED25519Type)
	require.Contains(t, err.Error(), "failed to marshal private key")

	remoteKMS.marshalFunc = failingMarshal

	_, _, err = remoteKMS.ImportPrivateKey(privateKey, kmsapi.ED25519Type)
	...
	t.Run("ImportPrivateKey API error", func(t *testing.T) {
		...
		_, _, err = tmpKMS.ImportPrivateKey(privateKey, kmsapi.ED25519Type)
		require.Contains(t, err.Error(), "api error msg")
	})
}
...
func processPOSTRequest(w http.ResponseWriter, r *http.Request, keysetID, kid string,
	defaultExportPubKey []byte, defaultExportKeyType kmsapi.KeyType) error {
	xRootCapabilityHeaderValue := []byte("DUMMY")
...
func processExportKeyRequest(w io.Writer, defaultExportPubKey []byte, defaultExportKeyType kmsapi.KeyType) error {
	resp := &exportKeyResp{
		PublicKey: defaultExportPubKey,
