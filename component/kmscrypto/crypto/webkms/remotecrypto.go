...
type wrapKeyReq struct {
	...
	APU             []byte               `json:"apu"`
	APV             []byte               `json:"apv"`
	RecipientPubKey *cryptoapi.PublicKey `json:"recipient_pub_key"`
	Tag             []byte               `json:"tag,omitempty"`
}

// wrapKeyResp serializable WrapKey response.
type wrapKeyResp struct {
	cryptoapi.RecipientWrappedKey
}

// unwrapKeyReq serializable UnwrapKey request.
type unwrapKeyReq struct {
	WrappedKey   cryptoapi.RecipientWrappedKey `json:"wrapped_key"`
	SenderPubKey *cryptoapi.PublicKey          `json:"sender_pub_key,omitempty"`
	Tag          []byte                        `json:"tag,omitempty"`
}
...
//	error in case of errors
func (r *RemoteCrypto) WrapKey(cek, apu, apv []byte, recPubKey *cryptoapi.PublicKey, // nolint:funlen
...
func (r *RemoteCrypto) buildWrappedKeyResponse(respBody []byte, dest string) (*cryptoapi.RecipientWrappedKey, error) {
...
//	error in case of errors
func (r *RemoteCrypto) UnwrapKey(recWK *cryptoapi.RecipientWrappedKey, keyURL interface{},
...
func ksToCryptoPublicKey(ks interface{}) (*cryptoapi.PublicKey, error) {
	switch kst := ks.(type) {
	case *keyset.Handle:
