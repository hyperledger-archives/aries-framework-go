...
func TestWrapUnWrapKey(t *testing.T) {
	...
	t.Run("Wrap/Unwrap successful with Sender key (authcrypt)", func(t *testing.T) {
		wrappedKey, err := rCrypto.WrapKey(cek, apu, apv, recipentPubKey, cryptoapi.WithSender(senderKID))
		require.NoError(t, err)

		dCEK, err := rCrypto.UnwrapKey(wrappedKey, defaultKeyURL, cryptoapi.WithSender(&cryptoapi.PublicKey{}))
		require.NoError(t, err)
		require.EqualValues(t, cek, dCEK)
	})

	t.Run("Wrap/Unwrap with Sender key option containing empty key", func(t *testing.T) {
		wrappedKey, err := rCrypto.WrapKey(cek, apu, apv, recipentPubKey, cryptoapi.WithSender(""))
		require.NoError(t, err)
	...
	t.Run("Wrap/Unwrap with Sender key option containing nil key", func(t *testing.T) {
		wrappedKey, err := rCrypto.WrapKey(cek, apu, apv, recipentPubKey, cryptoapi.WithSender(nil))
		require.NoError(t, err)

		dCEK, err := rCrypto.UnwrapKey(wrappedKey, defaultKeyURL, cryptoapi.WithSender(nil))
		require.NoError(t, err)
		require.EqualValues(t, cek, dCEK)
	})
}
...
func TestRemoteCryptoWithHeadersFunc(t *testing.T) {
	...
	t.Run("New remote crypto using WithHeaders with error function", func(t *testing.T) {
		...
		wKey := &cryptoapi.RecipientWrappedKey{
			KID:          "1",
...
func exportPubKey(kh *keyset.Handle) (*cryptoapi.PublicKey, error) {
	pubKH, err := kh.Public()
	if err != nil {
...
func wrapKeyPostHandle(w http.ResponseWriter, reqBody []byte, senderKH *keyset.Handle, cr cryptoapi.Crypto) error {
	wrapReq := &wrapKeyReq{}
...
func buildRecipientWK(cek, apu, apv []byte, senderKH *keyset.Handle, wrapReq *wrapKeyReq,
	cr cryptoapi.Crypto) (*cryptoapi.RecipientWrappedKey, error) {
	var (
		wk  *cryptoapi.RecipientWrappedKey
		opt cryptoapi.WrapKeyOpts
		err error
	)

	if senderKH != nil {
		opt = cryptoapi.WithSender(senderKH)
...
func TestNonOKStatusCode(t *testing.T) {
	...
	t.Run("UnwrapKey Post request failure 501", func(t *testing.T) {
		...
		_, err := tmpCrypto.UnwrapKey(&cryptoapi.RecipientWrappedKey{}, nonOKDefaultKeyURL)
		require.Contains(t, err.Error(), "501")
	})
