...
func TestExtractSDJWTVersion(t *testing.T) {
	type args struct {
		isSDJWT     bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ExtractSDJWTVersion(tt.args.isSDJWT, tt.args.joseHeaders); got != tt.want {
				t.Errorf("ExtractSDJWTVersion() = %v, want %v", got, tt.want)
			}
		})
	}
}
