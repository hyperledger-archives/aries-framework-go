...
func TestWrapKey_Failure(t *testing.T) {
	...
	badRecipientKey := &cryptoapi.PublicKey{
		Type: ecdhpb.KeyType_UNKNOWN_KEY_TYPE.String(),
	}

	_, err = c.WrapKey(cek, apu, apv, badRecipientKey, cryptoapi.WithSender(recipientKey))
...
func TestUnwrapKey_Failure(t *testing.T) {
	...
	wk, err := validCrypter.WrapKey(cek, apu, apv, okpRecipientKey, cryptoapi.WithSender(okpSenderKH),
		cryptoapi.WithXC20PKW())
	require.NoError(t, err)

	badWK := &cryptoapi.RecipientWrappedKey{
		KID:          wk.KID,
		EncryptedCEK: wk.EncryptedCEK,
		EPK:          cryptoapi.PublicKey{Type: ecdhpb.KeyType_UNKNOWN_KEY_TYPE.String()},
		Alg:          wk.Alg,
		APU:          wk.APU,
		APV:          wk.APV,
	}
	...
	_, err = validCrypter.UnwrapKey(badWK, okpRecipientKH, cryptoapi.WithSender(okpSenderKey))
	...
	_, err = validCrypter.UnwrapKey(wk, okpRecipientKH, cryptoapi.WithSender([]byte("badSenderKey")))
	require.EqualError(t, err, "unwrapKey: deriveKEKAndUnwrap: error ECDH-1PU kek derivation: derive1PUKEKForUnwrap:"+
	...
	k, err := validCrypter.UnwrapKey(wk, okpRecipientKH, cryptoapi.WithSender(okpSenderKey))
	require.NoError(t, err)
	require.EqualValues(t, cek, k)

	// unwrap with valid Sender key as *keyset.Handle should pass
	k, err = validCrypter.UnwrapKey(wk, okpRecipientKH, cryptoapi.WithSender(okpSenderPublicKH))
	require.NoError(t, err)
	require.EqualValues(t, cek, k)
	...
	_, err = validCrypter.UnwrapKey(wk, okpRecipientKH, cryptoapi.WithSender(badPrivHK))
	require.EqualError(t, err, "unwrapKey: deriveKEKAndUnwrap: error ECDH-1PU kek derivation: derive1PUKEKForUnwrap:"+
		" OKP key derivation error derive1PUWithOKPKeyForUnwrap: failed to retrieve sender key: ksToPublicX25519Key: "+
