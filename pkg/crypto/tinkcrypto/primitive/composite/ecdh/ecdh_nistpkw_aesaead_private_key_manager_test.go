...
func TestECDHNISTPAESPrivateKeyManager_Primitive(t *testing.T) {
	...
	t.Run("Test private key manager Primitive() with empty serialized key", func(t *testing.T) {
		...
		require.EqualError(t, err, errInvalidECDHNISTPAESPrivateKey.Error(),
			"ecdhNISTPAESPrivateKeyManager primitive from empty serialized key must fail")
		require.Empty(t, p)
	})
	...
	t.Run("Test private key manager Primitive() with bad serialize key", func(t *testing.T) {
		...
		require.EqualError(t, err, errInvalidECDHNISTPAESPrivateKey.Error(),
			"ecdhNISTPAESPrivateKeyManager primitive from bad serialized key must fail")
		require.Empty(t, p)
	})
	...
	for _, tc := range flagTests {
		...
		t.Run("Test "+tt.tcName, func(t *testing.T) {
			...
			if bytes.Equal(tt.encTmp.Value, badSerializedFormat) {
				require.EqualError(t, err, errInvalidECDHNISTPAESPrivateKey.Error(),
					"ecdhNISTPAESPrivateKeyManager primitive from serialized key with invalid serialized key")
				require.Empty(t, p)
...
func TestEcdhNISTPAESPrivateKeyManager_DoesSupport(t *testing.T) {
	...
	require.True(t, km.DoesSupport(ecdhNISTPAESPrivateKeyTypeURL))
}
...
func TestEcdhNISTPAESPrivateKeyManager_NewKey(t *testing.T) {
	...
	t.Run("Test private key manager NewKey() with nil key", func(t *testing.T) {
		k, err := km.NewKey(nil)
		require.EqualError(t, err, errInvalidECDHNISTPAESPrivateKeyFormat.Error())
		require.Empty(t, k)
	})
	...
	for _, tc := range flagTests {
		...
		t.Run("Test "+tt.tcName, func(t *testing.T) {
			...
			if strings.Contains(tt.tcName, "success") {
				...
				require.Equal(t, kd.TypeUrl, ecdhNISTPAESPrivateKeyTypeURL)
			...
			if bytes.Equal(tt.encTmp.Value, badSerializedFormat) {
				require.EqualError(t, err, errInvalidECDHNISTPAESPrivateKeyFormat.Error(),
					"ecdhNISTPAESPrivateKeyManager NewKey from serialized key with invalid serialized key")
				require.Empty(t, p)
