...
func TestECDHNISTPXChachaPublicKeyManager_Primitive(t *testing.T) {
	km := newECDHNISTPXChachaPublicKeyManager()

	t.Run("Test public key manager Primitive() with empty serialized key", func(t *testing.T) {
		p, err := km.Primitive([]byte(""))
		require.EqualError(t, err, errInvalidECDHNISTPXChachaPublicKey.Error(),
			"ecdhNISTPXChachaPublicKeyManager primitive from empty serialized key must fail")
		require.Empty(t, p)
	})
	...
	t.Run("Test public key manager Primitive() with bad serialize key", func(t *testing.T) {
		...
		require.EqualError(t, err, errInvalidECDHNISTPXChachaPublicKey.Error(),
			"ecdhNISTPXChachaPublicKeyManager primitive from bad serialized key must fail")
		require.Empty(t, p)
	})
	...
	for _, tc := range flagTests {
		...
		t.Run("Test "+tt.tcName, func(t *testing.T) {
			...
			if strings.Contains(tt.tcName, "with bad content encryption key size") {
				require.EqualError(t, err, errInvalidECDHNISTPXChachaPublicKey.Error(),
					"ecdhNISTPXChachaPublicKeyManager primitive from serialized key with invalid serialized key")
				require.Empty(t, p)
...
func TestEcdhNISTPXChachaPublicKeyManager_DoesSupport(t *testing.T) {
	km := newECDHNISTPXChachaPublicKeyManager()
	require.False(t, km.DoesSupport("bad/url"))
	require.True(t, km.DoesSupport(ecdhNISTPXChachaPublicKeyTypeURL))
}

func TestEcdhNISTPXChachaPublicKeyManager_NewKeyAndNewKeyData(t *testing.T) {
	km := newECDHNISTPXChachaPublicKeyManager()

	t.Run("Test public key manager NewKey()", func(t *testing.T) {
		k, err := km.NewKey(nil)
		require.EqualError(t, err, "ecdh_nistpkw_xchachaaead_public_key_manager: NewKey not implemented")
		require.Empty(t, k)
	})

	t.Run("Test private key manager NewKeyData()", func(t *testing.T) {
		p, err := km.NewKeyData(nil)
		require.EqualError(t, err, "ecdh_nistpkw_xchachaaead_public_key_manager: NewKeyData not implemented")
		require.Empty(t, p)
	})
}
