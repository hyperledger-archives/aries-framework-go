...
func TestECDHX25519XChachaPublicKeyManager_Primitive(t *testing.T) {
	km := newECDHX25519XChachaPublicKeyManager()

	t.Run("Test public key manager Primitive() with empty serialized key", func(t *testing.T) {
		p, err := km.Primitive([]byte(""))
		require.EqualError(t, err, errInvalidECDHX25519XChachaPublicKey.Error(),
			"ecdhX25519XChachaPublicKeyManager primitive from empty serialized key must fail")
		require.Empty(t, p)
	})
	...
	t.Run("Test public key manager Primitive() with bad serialize key", func(t *testing.T) {
		...
		require.EqualError(t, err, errInvalidECDHX25519XChachaPublicKey.Error(),
			"ecdhX25519XChachaPublicKeyManager primitive from bad serialized key must fail")
		require.Empty(t, p)
	})
	...
	for _, tc := range flagTests {
		...
		t.Run("Test "+tt.tcName, func(t *testing.T) {
			...
			if strings.Contains(tt.tcName, "with bad content encryption key size") {
				require.EqualError(t, err, errInvalidECDHX25519XChachaPublicKey.Error(),
					"ecdhX25519XChachaPublicKeyManager primitive from serialized key with invalid serialized key")
				require.Empty(t, p)
...
func TestEcdhX25519XChachaPublicKeyManager_DoesSupport(t *testing.T) {
	...
	require.True(t, km.DoesSupport(ecdhX25519XChachaPublicKeyTypeURL))
}
...
func TestEcdhX25519XChachaPublicKeyManager_NewKeyAndNewKeyData(t *testing.T) {
	...
	t.Run("Test public key manager NewKey()", func(t *testing.T) {
		k, err := km.NewKey(nil)
		require.EqualError(t, err, "ecdh_x25519kw_xchachaaead_public_key_manager: NewKey not implemented")
		require.Empty(t, k)
	})

	t.Run("Test private key manager NewKeyData()", func(t *testing.T) {
		p, err := km.NewKeyData(nil)
		require.EqualError(t, err, "ecdh_x25519kw_xchachaaead_public_key_manager: NewKeyData not implemented")
		require.Empty(t, p)
	})
}
