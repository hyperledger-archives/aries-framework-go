...
func TestHKDFBasic(t *testing.T) {
	for ti, test := range hkdfTests {
		k, err := hex.DecodeString(test.key)
		if err != nil {
			t.Errorf("decode key string failed: %s", err)
		}

		s, err := hex.DecodeString(test.salt)
		if err != nil {
			t.Errorf("decode salt string failed: %s", err)
		}

		i, err := hex.DecodeString(test.info)
		if err != nil {
			t.Errorf("decode info string failed: %s", err)
		}
...
func TestNewHMACWithInvalidInput(t *testing.T) {
	...
	require.Errorf(t, err, "expect an error when hash algorithm is invalid")
	require.Containsf(t, err.Error(), "invalid hash algorithm",
		"expect an error when hash algorithm is invalid")

	// tag too short
	_, err = ComputeHKDF("SHA256", random.GetRandomBytes(16), nil, nil, 9)
	require.Errorf(t, err, "expect an error when tag size is too small")
	require.Containsf(t, err.Error(), "tag size too small",
		"expect an error when tag size is too small")

	// tag too big
	_, err = ComputeHKDF("SHA1", random.GetRandomBytes(16), nil, nil, 5101)
	require.Errorf(t, err, "expect an error when tag size is too big")
	require.Containsf(t, err.Error(), "tag size too big",
