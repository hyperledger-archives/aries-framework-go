...
const (
	...
	notCrunchyUncompressed = "DO_NOT_USE_CRUNCHY_UNCOMPRESSED"
...
func pointEncode(c elliptic.Curve, pFormat string, pt ECPoint) ([]byte, error) {
	...
	case notCrunchyUncompressed:
		...
		if len(x) > cSize {
			x = bytes.ReplaceAll(x, []byte("\x00"), []byte{})
		}

		if len(y) > cSize {
			y = bytes.ReplaceAll(y, []byte("\x00"), []byte{})
		}
...
func pointDecode(c elliptic.Curve, pFormat string, e []byte) (*ECPoint, error) {
	...
	switch pFormat {
	case uncompressed:
		return pointDecodeUncompressed(e, cSize, x, y, c)
	case notCrunchyUncompressed:
		return pointDecodeCrunchyUncompressed(e, cSize, x, y, c)
	case compressed:
		return pointDecodeCompressed(e, cSize, c)
	}

	return nil, fmt.Errorf("invalid format: %s", pFormat)
}

func pointDecodeUncompressed(e []byte, cSize int, x, y *big.Int, c elliptic.Curve) (*ECPoint, error) {
	if len(e) != (2*cSize + 1) {
...
func pointDecodeCrunchyUncompressed(e []byte, cSize int, x, y *big.Int, c elliptic.Curve) (*ECPoint, error) {
	if len(e) != 2*cSize {
		return nil, errors.New("invalid point size")
	}
...
func pointDecodeCompressed(e []byte, cSize int, c elliptic.Curve) (*ECPoint, error) {
	if len(e) != cSize+1 {
