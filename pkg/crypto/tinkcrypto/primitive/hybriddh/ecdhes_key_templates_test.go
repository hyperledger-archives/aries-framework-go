...
func TestECDHESAES128GCMKeyTemplateKeyTemplate(t *testing.T) {
	kformat := new(eciespb.EciesAeadHkdfKeyFormat)

	kt := ECDHESAES128GCMKeyTemplate()
	if kt.TypeUrl != ecdhesPrivateKeyTypeURL {
	...
	err := proto.Unmarshal(kt.Value, kformat)
	if err != nil {
...
func TestECDHESAES256GCMKeyTemplateKeyTemplate(t *testing.T) {
	kformat := new(eciespb.EciesAeadHkdfKeyFormat)

	kt := ECDHESAES256GCMKeyTemplate()
	if kt.TypeUrl != ecdhesPrivateKeyTypeURL {
		t.Error("type url mismatch")
	}
	...
	if strings.Compare(kformat.Params.DemParams.AeadDem.String(), aead.AES256GCMKeyTemplate().String()) != 0 {
		t.Error("AEAD DEM mismatch")
	}
