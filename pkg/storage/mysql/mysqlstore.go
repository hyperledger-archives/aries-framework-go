...
func (s *sqlDBStore) Put(k string, v []byte) error {
	...
	//nolint: gosec create upsert query to insert the record, checking whether the key is already mapped to a value in the store.
...
func (s *sqlDBStore) Get(k string) ([]byte, error) {
	...
	var value []byte
	// nolint: gosec select query to fetch the record by key
...
func (s *sqlDBStore) Delete(k string) error {
	...
	// nolint: gosec delete query to delete the record by key
...
func (s *sqlDBStore) Iterator(startKey, endKey string) storage.StoreIterator {
	...
	//nolint:gosec sub query to fetch the all the keys that have start and end key reference, simulating range behavior.
