...
func (s *Service) invitationMsgRecord(msg service.DIDCommMsg) (*connection.Record, error) {
	...
	var (
		sp             model.Endpoint
		didCommVersion service.Version
	)

	if isDIDCommV2(s.ctx.mediaTypeProfiles) {
		sp = model.NewDIDCommV2Endpoint([]model.DIDCommV2Endpoint{
			{URI: invitation.ServiceEndpoint, Accept: s.ctx.mediaTypeProfiles},
		})
		didCommVersion = service.V2
	} else {
		sp = model.NewDIDCommV1Endpoint(invitation.ServiceEndpoint)
		didCommVersion = service.V1
	}
