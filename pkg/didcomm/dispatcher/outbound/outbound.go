...
func (o *Dispatcher) SendToDID(msg interface{}, myDID, theirDID string) error { // nolint:funlen,gocyclo,gocognit
	...
	case transport.MediaTypeAIP2RFC0587Profile, transport.MediaTypeDIDCommV2Profile:
		var (
			url                 string
			accept, routingKeys []string
		)

		url, err = connRec.ServiceEndPoint.URI()
		if err != nil && !strings.EqualFold(err.Error(), "endpoint URI not found") {
			return err
		}

		accept, err = connRec.ServiceEndPoint.Accept()
		if err != nil && !strings.EqualFold(err.Error(), "endpoint Accept not found") {
			return err
		}

		routingKeys, err = connRec.ServiceEndPoint.RoutingKeys()
		if err != nil && !strings.EqualFold(err.Error(), "endpoint RoutingKeys not found") {
			return err
		}

		dest.ServiceEndpoint = commonmodel.NewDIDCommV2Endpoint([]commonmodel.DIDCommV2Endpoint{{
			URI:         url,
			Accept:      accept,
