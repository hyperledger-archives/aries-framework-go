...
func (bp *Packager) prepareSenderAndRecipientKeys(cty string, envelope *transport.Envelope) ([]byte, [][]byte, error) {
	...
	for i, receiverKey := range envelope.ToKeys {
		if strings.HasPrefix(receiverKey, "did:key") { //nolint:gocritic,nestif
		...
		} else if strings.Index(receiverKey, "#") > 0 {
	...
	if strings.HasPrefix(string(envelope.FromKey), "did:key") { //nolint:nestif
	...
	} else if i := bytes.Index(envelope.FromKey, []byte("#")); i > 0 {
