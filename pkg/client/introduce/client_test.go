...
func TestClient_SendProposal(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		svc := serviceMocks.NewMockDIDComm(ctrl)
		svc.EXPECT().
			HandleOutbound(gomock.Any(), "firstMyDID", "firstTheirDID").
			DoAndReturn(func(msg service.DIDCommMsg, myDID, theirDID string) error {
...
func TestClient_SendProposalWithInvitation(t *testing.T) {
	...
	require.NoError(t, client.SendProposalWithInvitation(&didexchange.Invitation{}, &introduce.Recipient{
		MyDID:    "firstMyDID",
		TheirDID: "firstTheirDID",
	}))
}
