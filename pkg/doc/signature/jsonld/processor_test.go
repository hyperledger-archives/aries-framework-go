	...
	t.Run("Test get canonical document", func(t *testing.T) {
		tests := []struct {
		...
		}{
			{
				...
				opts:   []jsonld.ProcessorOpts{jsonld.WithRemoveAllInvalidRDF()},
			},
			{
				name:   "canonizing document with 1 incorrect RDF",
				doc:    jsonLDWithIncorrectRDF,
			...
			},
			{
				...
				opts: []jsonld.ProcessorOpts{
					jsonld.WithRemoveAllInvalidRDF(),
					jsonld.WithExternalContext("https://trustbloc.github.io/context/vc/examples-v1.jsonld"),
				},
			},
			{
				...
				opts: []jsonld.ProcessorOpts{
					jsonld.WithRemoveAllInvalidRDF(),
					jsonld.WithExternalContext("http://localhost:8652/dummy.jsonld"),
				},
			},
			{
			...
			},
			{
				...
				doc:    vcWithProperContexts2,
				result: canonizedJSONCredential2,
				opts:   []jsonld.ProcessorOpts{jsonld.WithDocumentLoader(loader)},
			},
			{
			...
			},
			{
				...
				doc:    jsonLDWith2KnownInvalidRDFs,
				result: canonizedIncorrectRDFAllFiltered,
				opts:   []jsonld.ProcessorOpts{jsonld.WithValidateRDF()},
				err:    jsonld.ErrInvalidRDFFound.Error(),
			},
			{
				name:   "canonizing document with 1 incorrect RDF with validation & remove all invalid RDF option",
				doc:    jsonLDWith2KnownInvalidRDFs,
				result: canonizedIncorrectRDFAllFiltered,
				opts:   []jsonld.ProcessorOpts{jsonld.WithValidateRDF(), jsonld.WithRemoveAllInvalidRDF()},
				err:    jsonld.ErrInvalidRDFFound.Error(),
			},
		}
		...
		for _, test := range tests {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				response, err := jsonld.NewProcessor(defaultAlgorithm).GetCanonicalDocument(jsonldDoc,
					append([]jsonld.ProcessorOpts{jsonld.WithDocumentLoader(loader)}, tc.opts...)...)
				if tc.err != "" {
					require.Error(t, err)
...
func TestTransformBlankNodes(t *testing.T) {
	...
	at := jsonld.TransformBlankNode(a)
	require.Equal(t, ae, at)

	bt := jsonld.TransformBlankNode(b)
	require.Equal(t, be, bt)

	ct := jsonld.TransformBlankNode(c)
	require.Equal(t, ce, ct)

	dt := jsonld.TransformBlankNode(d)
	require.Equal(t, de, dt)

	et := jsonld.TransformBlankNode(e)
	require.Equal(t, ee, et)

	ft := jsonld.TransformBlankNode(f)
	require.Equal(t, fe, ft)

	gt := jsonld.TransformBlankNode(g)
