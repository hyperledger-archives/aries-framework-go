...
func TestNewDocumentLoader(t *testing.T) {
	t.Run("Preload default contexts", func(t *testing.T) {
		storageProvider := mockstorage.NewMockStoreProvider()

		loader, err := jsonld.NewDocumentLoader(storageProvider, jsonld.WithContexts(jsonld.DefaultContexts...))

		require.NotNil(t, loader)
		require.NoError(t, err)
		require.Equal(t, len(jsonld.DefaultContexts), len(storageProvider.Store.Store))
	})
