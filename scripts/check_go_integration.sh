...
go test -count=1 -v -cover . -p 1 -timeout=30m -race -run presentproof,present_proof_controller,issue_credential
go test -count=1 -v -cover . -p 1 -timeout=30m -race -run didcomm_remote_crypto,verifiable,issue_credential_controller,webkms,waci_issuance
