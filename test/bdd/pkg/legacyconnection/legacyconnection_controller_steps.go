...
const (
	connOperationID              = "/legacy/connections"
	kmsOperationID               = "/kms"
...
var logger = log.New("aries-framework/legacy-connection-tests")

// ControllerSteps is steps for legacy-connection with controller.
type ControllerSteps struct {
	bddContext            *context.BDDContext
	invitations           map[string]*legacyconnection.Invitation
...
func NewLegacyConnectionControllerSteps() *ControllerSteps {
	return &ControllerSteps{
		invitations:   make(map[string]*legacyconnection.Invitation),
...
func (a *ControllerSteps) RegisterSteps(s *godog.Suite) {
	s.Step(`^"([^"]*)" creates legacy invitation through controller with label "([^"]*)"$`, a.createInvitation)
	s.Step(`^"([^"]*)" creates legacy invitation through controller with label "([^"]*)" and router "([^"]*)"$`,
		a.createInvitationWithRouter)
	s.Step(`^"([^"]*)" receives legacy invitation from "([^"]*)" through controller$`, a.receiveInvitation)
	s.Step(`^"([^"]*)" approves connection invitation through controller$`, a.approveInvitation)
	s.Step(`^"([^"]*)" approves connection invitation with router "([^"]*)" through controller$`,
		a.approveInvitationWithRouter)
	s.Step(`^"([^"]*)" approves connection request through controller`, a.ApproveRequest)
	s.Step(`^"([^"]*)" approves connection request with router "([^"]*)" through controller$`,
		a.ApproveRequestWithRouter)
	s.Step(`^"([^"]*)" waits for legacy post state event "([^"]*)" to web notifier`, a.WaitForPostEvent)
	s.Step(`^"([^"]*)" retrieves connection record through controller and validates that legacy connection state is "([^"]*)"$`, a.validateConnections) //nolint: lll
	// public DID steps
	s.Step(`^"([^"]*)" creates "([^"]*)" public DID through legacy controller`, a.CreatePublicDID)
	s.Step(`^"([^"]*)" creates legacy invitation through controller using public DID and label "([^"]*)"$`,
		a.createInvitationWithDID)
	s.Step(`^"([^"]*)" approves legacy invitation with public DID through controller`,
		a.approveInvitationWithPublicDID)
	s.Step(`^"([^"]*)" approves connection request with public DID through controller`,
		a.approveRequestWithPublicDID)
	s.Step(`^"([^"]*)" initiates legacy connection through controller with "([^"]*)" using peer DID$`,
		a.createImplicitInvitation)
	s.Step(`^"([^"]*)" initiates legacy connection through controller with "([^"]*)" using public DID$`,
		a.createImplicitInvitationWithDID)
	s.Step(`^"([^"]*)" has established connection with "([^"]*)" through legacy connection using controller$`,
		a.performLegacyConnection)
...
func (a *ControllerSteps) EstablishConnection(inviter, invitee string) error {
	return a.performLegacyConnection(inviter, invitee)
}
...
func (a *ControllerSteps) pullEventsFromWebSocket(agentID, state string) (string, error) {
	msg, err := util.PullEventsFromWebSocket(a.bddContext, agentID,
		util.FilterTopic("legacyconnection_states"),
...
func (a *ControllerSteps) performCreateInvitation(inviterAgentID, label, routerID string, useDID bool) error {
	...
	var result legconncmd.CreateInvitationResponse
...
func (a *ControllerSteps) performCreateImplicitInvitation(inviteeAgentID, inviterAgentID string, usePublic bool) error {
	...
	var result legconncmd.ImplicitInvitationResponse
...
func (a *ControllerSteps) verifyCreateInvitationResult(result *legconncmd.CreateInvitationResponse,
...
func (a *ControllerSteps) receiveInvitation(inviteeAgentID, inviterAgentID string) error {
	...
	// call controller
	var result legconncmd.ReceiveInvitationResponse
...
func (a *ControllerSteps) performApproveInvitation(agentID, routerID string, useDID bool) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("approve connection invitation : %w", err)
	}

	// invitee connectionID
	a.connectionIDs[agentID] = connectionID

	var response legconncmd.AcceptInvitationResponse
...
func (a *ControllerSteps) performApproveRequest(agentID, routerID string, useDID bool) (string, error) {
	...
	var response legconncmd.ConnectionResponse
...
func (a *ControllerSteps) validateConnection(agentID, stateValue string) error {
	...
	// call controller
	var response legconncmd.QueryConnectionResponse
...
func (a *ControllerSteps) verifyConnectionList(agentID, queryState, verifyID string) error {
	...
	// call controller
	var response legconncmd.QueryConnectionsResponse
...
func (a *ControllerSteps) CreatePublicDIDWithKeyType(agentID, keyType string) (string, error) { //nolint:funlen,gocyclo
	destination, ok := a.bddContext.GetControllerURL(agentID)
	if !ok {
		return "", fmt.Errorf(" unable to find controller URL registered for agent [%s]", agentID)
	}
...
func (a *ControllerSteps) performLegacyConnection(inviter, invitee string) error {
