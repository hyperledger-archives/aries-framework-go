...
Feature: Messaging between the agents using SDK
  ...
  Scenario: sending message from one agent to another using message service
    Given "Filip" uses a message registrar
    ...
    Given "Derek" uses a message registrar
      And "Derek" is started with a "http" DIDComm endpoint

    Given "Derek" registers a message service with name "generic-invite-response" for type "https://didcomm.org/generic/1.0/message" and purpose "invite-response"
  ...
  Scenario: sending message from one agent to another using message service using "Basic Message Protocol 1.0"
    ...
    Given "Derek" is started with a "http" DIDComm endpoint
    And   "Derek" creates did exchange client
