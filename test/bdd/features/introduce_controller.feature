...
Feature: Introduce using controller API
  ...
  Scenario: Jacob has a Carol's public out-of-band request. The protocol starts with introduce request.
    ...
    And   "Jacob" sends introduce proposal back to the requester with public out-of-band request through the controller
  ...
  Scenario: William sends a response with approve and an out-of-band request.
    ...
    And   "William" wants to know "Olivia" and sends introduce response with approve and provides an out-of-band request through the controller
  ...
  Scenario: Mason sends a response with approve and an out-of-band request. The protocol starts with introduce request
    ...
    And   "Mason" wants to know "Sophia" and sends introduce response with approve and provides an out-of-band request through the controller
